<template>
    <app-layout>
        ...
        <div class=" mx-auto mb-4 flex justify-between items-center">
            ...
            <inertia-link v-if="can('legal_types.create')" class="btn btn-blue" :href="route('legal_types.create')">
                <span>Create </span>
                    ...
                    <tr v-for="type in types.data" :key="type.id"
                        ...
                        <td class="border-t w-px pr-2">
                            <div class=" flex items-center space-x-2">
                                <inertia-link v-if="can('legal_types.update')" :href="route('legal_types.edit', type.id)"
                                              tabindex="-1" class="text-maiic-600 hover:text-maiic-900">
                                    Edit
                                </inertia-link>
                                <a href="#" v-if="can('legal_types.destroy')" @click="deleteAction(type.id)"
...
export default {
    ...
    watch: {
        form: {
            handler: _.debounce(function () {
                ...
                this.$inertia.get(this.route('legal_types.index', Object.keys(query).length ? query : {}))
            }, 500),
            deep: true,
        },
    },
    methods: {
        ...
        destroy() {
            ...
            this.$inertia.delete(this.route('legal_types.destroy', this.selectedRecord))
