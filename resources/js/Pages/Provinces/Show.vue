    ...
    <app-layout>
        <template #header>
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">
                <inertia-link class="text-maiic-500 hover:text-maiic-600" :href="route('locations.provinces.index')">
                    Provinces
                </inertia-link>
                <span class="text-maiic-500 font-medium">/</span> Province #{{ province.id }}
            </h2>
        </template>
                    ...
                    <div class="intro-y bg-white mt-5 lg:mt-0">
                        <div class="relative flex items-center p-5">
                            <div class="ml-4 mr-auto">
                                <div class="font-medium text-base">Province #{{ province.id }}</div>
                            </div>
                            <jet-dropdown align="right" width="60">
                                ...
                                <template #content>
                                    <div class="w-60 shadow-xl bg-white rounded">
                                        <jet-dropdown-link :href="route('locations.provinces.edit',province.id)">
                ...
                <div class="w-full md:w-9/12  md:ml-4">
                    <div class="bg-white p-5">
                        <div class="flex justify-between">
                            <h3>Districts</h3>
                            <inertia-link class="btn btn-blue" v-if="can('locations.create')" :href="route('locations.districts.create',{province_id:province.id})">
                                <span>Create </span>
                        ...
                        <div class=" overflow-x-auto">
                            <table class="w-full whitespace-no-wrap mt-4">
                                    ...
                                    <td class="border-t w-px pr-2">
                                        <div class=" flex items-center space-x-2">
                                            <inertia-link v-if="can('locations.index')"
                                                          :href="route('locations.districts.show', district.id)"
                                                          tabindex="-1" class="text-maiic-600 hover:text-maiic-900">
                                                View
                                            </inertia-link>
                                            <inertia-link v-if="can('locations.update')"
                                                          :href="route('locations.districts.edit', district.id)"
                                                          tabindex="-1" class="text-maiic-600 hover:text-maiic-900">
                                                Edit
                                            </inertia-link>
                                ...
                                <tr v-if="province.districts.length === 0">
                                    <td class="border-t px-6 py-4 text-center" colspan="2">No districts found.</td>
                                </tr>
                                </tbody>
                            </table>
                        </div>
...
export default {
    ...
    data() {
        return {
            ...
            selectedRecord: null,
            processing: false,
            pageTitle: "Province Details",
            pageDescription: "Province Details",
        }
    ...
    methods: {
        ...
        destroy() {
            this.$inertia.delete(this.route('locations.provinces.destroy', this.selectedRecord))
            this.confirmingDeletion = false
            window.location = route('locations.provinces.index')
        },
        ...
        destroyDistrict() {
            this.$inertia.delete(this.route('locations.districts.destroy', this.selectedDistrictRecord))
