<template>
    <app-layout>
        ...
        <div class=" mx-auto mb-4 flex justify-between items-center">
            ...
            <inertia-link v-if="can('locations.create')" class="btn btn-blue" :href="route('locations.provinces.create')">
                <span>Create </span>
                <span class="hidden md:inline">Province</span>
            </inertia-link>
        </div>
        <div class=" mx-auto">
                    ...
                    <tr v-for="province in provinces.data" :key="province.id"
                        ...
                        <td class="border-t w-px pr-2">
                            <div class=" flex items-center space-x-2">
                                <inertia-link v-if="can('locations.index')" :href="route('locations.provinces.show', province.id)"
                                              tabindex="-1" class="text-maiic-600 hover:text-maiic-900">
                                    View
                                </inertia-link>
                                <inertia-link v-if="can('locations.update')" :href="route('locations.provinces.edit', province.id)"
                                              tabindex="-1" class="text-maiic-600 hover:text-maiic-900">
                                    Edit
                                </inertia-link>
                                <a href="#" v-if="can('locations.destroy')" @click="deleteAction(province.id)"
...
export default {
    ...
    data() {
        return {
            ...
            selectedRecord: null,
            pageTitle: "Provinces",
            pageDescription: "Manage Provinces",
    ...
    watch: {
        form: {
            handler: _.debounce(function () {
                ...
                this.$inertia.get(this.route('locations.provinces.index', Object.keys(query).length ? query : {}))
            }, 500),
            deep: true,
        },
    },
