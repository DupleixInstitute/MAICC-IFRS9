<template>
    <Modal v-if="show" @close="$emit('close')">
        <div class="p-6">
            <h3 class="text-lg font-semibold text-gray-900 mb-4">Export Transition Matrix Report</h3>
    ...
    methods: {
        ...
        async submitExport() {
            ...
            try {
                ...
                const response = await fetch(route('transition-matrices.report-by-period') + '?' + params.toString(), {
                    method: 'GET',
                    headers: {
                ...
                let filename = `Transition_Matrix_Report_${this.form.start_period}_to_${this.form.end_period}.${this.form.compress_file ? 'zip' : this.form.format}`
                ...
                this.closeModal()
                this.$emit('success', 'Report exported successfully')
