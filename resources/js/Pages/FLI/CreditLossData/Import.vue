    ...
    <app-layout>
        <template #header>
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">
                <inertia-link class="text-indigo-400 hover:text-indigo-600" :href="route('credit-loss-data.index')">Credit Loss Data
                </inertia-link>
                ...
                <form @submit.prevent="submit">
                    <div class="space-y-6">
                        ...
                        <div v-if="showPreview" class="mt-6 border-t border-gray-200 pt-6">
                            <h4 class="text-sm font-medium text-gray-900 mb-4">Map CSV Headers</h4>
                            
                            <!-- Validation Summary -->
                            <div v-if="validationSummary" class="mb-4 p-4 bg-blue-50 rounded-lg">
                                <div class="flex items-center">
                                    <svg class="w-5 h-5 text-blue-500 mr-2" fill="currentColor" viewBox="0 0 20 20">
                                        <path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a1 1 0 000 2v3a1 1 0 001 1h1a1 1 0 100-2v-3a1 1 0 00-1-1H9z" clip-rule="evenodd"/>
                                    </svg>
                                    <span class="text-sm text-blue-700">
                                        Required fields mapped: {{ validationSummary.requiredMapped }}/{{ validationSummary.requiredTotal }}
                                        | Optional fields: {{ validationSummary.optionalMapped }}
                                    </span>
                                </div>
                                <div v-if="!validationSummary.isValid" class="mt-2 text-sm text-red-600">
                                ...
                                <table class="min-w-full divide-y divide-gray-300">
                                    <thead class="bg-gray-50">
                                        <tr>
                                            ...
                                            <th class="px-3 py-3.5 text-left text-sm font-semibold text-gray-900">Database Field</th>
                                        ...
                                        <tr v-for="(header, index) in previewData.headers" :key="index">
                                            ...
                                            <td class="whitespace-nowrap px-3 py-4 text-sm text-gray-900">
                                                <select 
                                                    ...
                                                    @change="updateFieldMapping(header, $event.target.value)"
                                                    :disabled="isProcessing"
                                                >
                                                    ...
                                                    <option 
                                                        ...
                                                        :key="field.name"
                                                        :value="field.name"
                                                        :disabled="field.isMapped && getMappedField(header) !== field.name"
                                                    >
                                                        {{ field.label }}
                                                    </option>
                                                </select>
                                        ...
                                        <thead class="bg-gray-50">
                                            <tr>
                                                <th 
                                                    ...
                                                    :class="{ 'bg-green-50': getMappedField(header) }"
                                                >
                                                    <div class="text-xs">{{ header }}</div>
                                                    <div class="text-xs text-gray-500 font-normal">
                                                        {{ getMappedField(header) || 'Unmapped' }}
                                                    </div>
                                                </th>
                                            </tr>
                                        </thead>
                ...
                <div class="space-y-6">
                    <div class="flex items-center justify-between">
                        <div class="flex items-center space-x-4">
                            ...
                            <span class="text-gray-700 font-medium">Importing data...</span>
                        </div>
        ...
        return {
            ...
            availableFields: [
                ...
                },
                {
                    ...
                    label: 'ECL Value',
                    required: false,
                    description: 'Expected Credit Loss value'
                },
                {
                    name: 'npl_value',
                    label: 'NPL Value',
                    required: false,
                    description: 'Non-Performing Loans value'
                },
                {
                    name: 'pd_value',
                    label: 'PD Value',
                ...
                },
                {
                    ...
                    label: 'Stage',
                    required: false,
                    description: 'Credit stage classification'
                },
                {
                    name: 'credit_rating',
                    label: 'Credit Rating',
    ...
    computed: {
        ...
        validationSummary() {
            ...
            const mappedRequired = requiredFields.filter(f => 
                Object.values(this.fieldMapping).includes(f.name)
            ).length;
            
            const optionalFields = this.availableFields.filter(f => !f.required);
            const mappedOptional = optionalFields.filter(f => 
                Object.values(this.fieldMapping).includes(f.name)
            ).length;
            ...
            return {
                ...
                optionalMapped: mappedOptional,
                isValid: mappedRequired === requiredFields.length
            };
        },
        canSubmit() {
            return this.form.file && this.form.loan_portfolio_id && this.form.reporting_period && this.validationSummary.isValid;
        }
    },

    methods: {
        submit() {
            // Add field mapping to form data
            this.form.field_mapping = this.fieldMapping;
        ...
        updateFieldMapping(header, fieldName) {
            if (fieldName === '') {
                // Remove mapping
