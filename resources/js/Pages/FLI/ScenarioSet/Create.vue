...
export default {
    ...
    data() {
        return {
            form: this.$inertia.form({
                ...
                is_active: true,
                scenarios: [
                    { scenario_name: 'Base Case', probability: 50 },
                    { scenario_name: 'Upside', probability: 20 },
                    { scenario_name: 'Downside', probability: 30 }
                ]
            }),
            pageTitle: "Create Scenario Set",
            pageDescription: "Create Economic Scenario Set",
        }
    },
    computed: {
        totalProbability() {
    ...
    methods: {
        ...
        submit() {
            ...
            this.form.post(this.route('fli.scenarios.store'), {
                onSuccess: () => {
                    this.$toast?.success('Scenario set created successfully!');
                },
                onError: () => {
                    this.$toast?.error('Failed to create scenario set. Please check your inputs.');
                }
            })
        },
    }
