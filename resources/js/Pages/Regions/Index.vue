<template>
    <app-layout>
        ...
        <div class=" mx-auto mb-4 flex justify-between items-center">
            ...
            <inertia-link v-if="can('locations.create')" class="btn btn-blue" :href="route('locations.regions.create')">
                <span>Create </span>
            ...
            <div class="bg-white rounded shadow overflow-x-auto">
                <table class="w-full whitespace-no-wrap">
                    ...
                    <tr v-for="province in provinces.data" :key="province.id"
                        ...
                        <td class="border-t w-px pr-2">
                            <div class=" flex items-center space-x-2">
                                <inertia-link v-if="can('locations.index')" :href="route('locations.regions.show', province.id)"
                                              tabindex="-1" class="text-maiic-600 hover:text-maiic-900">
                                    View
                                </inertia-link>
                                <inertia-link v-if="can('locations.update')" :href="route('locations.regions.edit', province.id)"
                                              tabindex="-1" class="text-maiic-600 hover:text-maiic-900">
                                    Edit
                                </inertia-link>
                    ...
                    <tr v-if="provinces.data.length === 0">
                        <td class="border-t px-6 py-4 text-center" colspan="3">No regions found.</td>
                    </tr>
                    </tbody>
                </table>
...
export default {
    ...
    data() {
        return {
            ...
            selectedRecord: null,
            pageTitle: "Regions",
            pageDescription: "Manage Regions",
    ...
    watch: {
        form: {
            handler: _.debounce(function () {
                ...
                this.$inertia.get(this.route('locations.regions.index', Object.keys(query).length ? query : {}))
            }, 500),
            deep: true,
        },
    },
