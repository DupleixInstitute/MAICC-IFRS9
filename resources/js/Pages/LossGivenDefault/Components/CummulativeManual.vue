<template>
    <div v-if="show" class="fixed inset-0 flex items-center justify-center bg-gray-800 bg-opacity-75">
        <div class="bg-white p-6 rounded-lg shadow-lg w-96">
            <h2 class="text-lg font-semibold">Manual Cummulative LGD Calculation</h2>
            ...
            <div v-if="mode === 'amount'" class="space-y-4 mt-4">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                        <jet-label for="periods_count" value="Periods Count" />
                        <input type="number" max="60" v-model="fields.periods_count"
                            class="mt-1 block w-full rounded-md border-gray-300 shadow-sm">
                    </div>
                </div>
            </div>
            ...
            <div v-else class="space-y-4 mt-4">
                <div>
                    ...
                    <input type="number" step="0.01" v-model="fields.cure_rate_cummulative"
                        class="mt-1 block w-full rounded-md border-gray-300 shadow-sm">
                </div>
                <div>
                    <jet-label for="recovery_rate" value="Recovery Rate (%)" />
                    <input type="number" step="0.01" v-model="fields.recovery_rate_cummulative"
                        class="mt-1 block w-full rounded-md border-gray-300 shadow-sm">
                </div>
                <div>
                    <jet-label for="periods_count" value="Periods Count" />
            ...
            <div class="flex justify-end mt-6">
                ...
                <Link :href="route('lgd-cummulative.index')" class="ml-2">Close</Link>
            </div>
        </div>
    </div>
</template>
    ...
    methods: {
        submitManualCalculation() {
            if (this.mode === 'amount') {
                ...
                const cureRate = parseFloat(this.fields.cure_rate_cummulative) / 100;
                const recoveryRate = parseFloat(this.fields.recovery_rate_cummulative) / 100;
                const lgd = (1 - cureRate) * (1 - recoveryRate);
                const periods = parseFloat(this.fields.periods_count);
