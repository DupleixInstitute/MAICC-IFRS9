    ...
    <app-layout>
        <template #header>
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">
                <inertia-link class="text-maiic-500 hover:text-maiic-600" :href="route('locations.wards.index')">
                    Wards
                </inertia-link>
                <span class="text-maiic-500 font-medium">/</span> Ward #{{ ward.id }}
            </h2>
        </template>
                    ...
                    <div class="intro-y bg-white mt-5 lg:mt-0">
                        <div class="relative flex items-center p-5">
                            <div class="ml-4 mr-auto">
                                <div class="font-medium text-base">Ward #{{ ward.id }}</div>
                            </div>
                            <jet-dropdown align="right" width="60">
                                ...
                                <template #content>
                                    <div class="w-60 shadow-xl bg-white rounded">
                                        <jet-dropdown-link :href="route('locations.wards.edit',ward.id)">
                                            <font-awesome-icon icon="edit"/>
                                            Edit
                                        </jet-dropdown-link>
                                        <a href="#"
                                           class="block px-4 py-2 text-sm leading-5 text-gray-700 hover:bg-gray-100 focus:outline-none focus:bg-gray-100 transition duration-150 ease-in-out"
                                           @click="deleteAction(ward.id)">
                ...
                <div class="w-full md:w-9/12  md:ml-4">
                    <div class="bg-white p-5">
                        <div class="flex justify-between">
                            <h3>Villages</h3>
                            <inertia-link class="btn btn-blue" v-if="can('locations.create')" :href="route('locations.villages.create',{ward_id:ward.id})">
                                <span>Create </span>
                                <span class="hidden md:inline">Village</span>
                            </inertia-link>
                        </div>
                        <div class=" overflow-x-auto">
                            <table class="w-full whitespace-no-wrap mt-4">
                                ...
                                <tbody>
                                <tr v-for="village in ward.villages" :key="village.id"
                                    ...
                                    <td class="border-t w-px pr-2">
                                        <div class=" flex items-center space-x-2">
                                            <inertia-link v-if="can('locations.index')"
                                                          :href="route('locations.villages.show', village.id)"
                                                          tabindex="-1" class="text-maiic-600 hover:text-maiic-900">
                                                View
                                            </inertia-link>
                                            <inertia-link v-if="can('locations.update')"
                                                          :href="route('locations.villages.edit', village.id)"
                                                          tabindex="-1" class="text-maiic-600 hover:text-maiic-900">
                                                Edit
                                            </inertia-link>
                                            <a href="#" v-if="can('locations.destroy')"
                                               @click="deleteItemAction(village.id)"
                                               class="text-red-600 hover:text-red-900">Delete</a>
                                        </div>
                                    </td>
                                </tr>
                                <tr v-if="ward.villages.length === 0">
                                    <td class="border-t px-6 py-4 text-center" colspan="2">No villages found.</td>
                                </tr>
                                </tbody>
                            </table>
                        </div>
...
export default {
    ...
    methods: {
        ...
        destroy() {
            this.$inertia.delete(this.route('locations.wards.destroy', this.selectedRecord))
            this.confirmingDeletion = false
            window.location = route('locations.wards.index')
        },
        deleteItemAction(id) {
