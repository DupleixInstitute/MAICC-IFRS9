...
export default {
    ...
    mounted() {
        this.form.invoice_items.forEach((item, index) => {
            let ref = 'item_tariff_' + index;
            this.$refs[ref].value=[{label:'tere',value:1}]
            this.$refs[ref].refreshOptions('in')
        });
        this.updateInvoice();
    },
    methods: {
        submit() {
            this.form.put(this.route('clients.invoices.update', this.invoice.id), {})
        },
        addService() {
