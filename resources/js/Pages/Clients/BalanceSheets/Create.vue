            ...
            <div class="md:flex md:items-start">
                ...
                <div class="w-full md:w-9/12 p-4 md:ml-4 bg-white sm:mt-4">
                    <div class="flex justify-between ">
                        <h2 class="font-semibold text-xl text-gray-800 leading-tight">Create Balance Sheet</h2>
...
export default {
    ...
    data() {
        return {
            ...
            pageTitle: "Create Balance Sheet",
            pageDescription: "Create Balance Sheet",

        }
    },
    mounted() {
        this.currentAssets.forEach(item => {
            this.form.charts.current_assets.push({
                chart_of_account_id: item.id,
                name: item.name,
                amount: ''
            })
        });
        this.otherAssets.forEach(item => {
            this.form.charts.other_assets.push({
                chart_of_account_id: item.id,
                name: item.name,
                amount: ''
            })
        });
        this.otherCurrentAssets.forEach(item => {
            this.form.charts.other_current_assets.push({
                chart_of_account_id: item.id,
                name: item.name,
                amount: ''
            })
        });
        this.fixedAssets.forEach(item => {
            this.form.charts.fixed_assets.push({
                chart_of_account_id: item.id,
                name: item.name,
                amount: ''
            })
        });
        this.currentLiabilities.forEach(item => {
            this.form.charts.current_liabilities.push({
                chart_of_account_id: item.id,
                name: item.name,
                amount: ''
            })
        });
        this.longTermLiabilities.forEach(item => {
            this.form.charts.long_term_liabilities.push({
                chart_of_account_id: item.id,
                name: item.name,
                amount: ''
            })
        });
        this.equity.forEach(item => {
            this.form.charts.equity.push({
                chart_of_account_id: item.id,
                name: item.name,
                amount: ''
            })
