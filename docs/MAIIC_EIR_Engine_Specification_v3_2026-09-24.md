...
| Moratorium "Principle Only" versus "Both", grace period | One shape only | `ScheduleGeneratorService.php` lines 64-69 capitalise every moratorium; `ContractMasterImport.php` line 103 maps the principal grace into the same column | `moratorium_type` and `grace_period_months` columns; generator produces both shapes; the import conflict is removed |
...
| Permissions | One broad permission | `permission:settings` on every EIR controller | `eir.view`, `eir.run`, `eir.export`, `eir.govern` (O15) |
...
|---|---|---|---|
| P1 Foundations | Governance Centre tables, service and screen; permissions; the repairs in 6.4; `loan_books.repayments` populated | nothing | T7 green; settings resolve by effective date in tests |
...
The next build session starts with P1 and P2, because neither waits on MAIIC: the Governance Centre, the repairs in 6.4, the reference-rate import using the repaired file, and the spread derivation tested against the nine loan books already held. P3 follows immediately, because the interest convention is proven and Extract C for 120 accounts is in hand. P4 to P7 are built against the teaching workbook's fixtures so that they are ready when the scheme settings arrive. Nothing waits idle on a data request.
