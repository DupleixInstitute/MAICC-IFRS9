...
> **2026-08-20 correction notice.** §3.5.2 and open item #25 carried a factual error in v2.3 — the `NAME` column on the 2026 fee spools was described as proof of a system field; MAIIC has confirmed it was hand-typed. Corrected in v2.4, together with the architectural consequence: **the GL cannot attribute either interest or fees to a customer, so the loan module is the sole source for both.** MAIIC has confirmed the open item #24 extract is in preparation.
...
**⚠️ CORRECTED 2026-08-20 — the `NAME` column is NOT a system field.** v2.3 of this spec claimed `GL 4871 as at 31 July 2026.xls` proved a structured `NAME` field existed in E-Banker. **That was wrong.** MAIIC confirmed in writing (Tamanda Sitimawina, 2026-08-20) that *"the GL 4871 and 4873 reports for 2026 were manually updated with the NAME column. I was trying to just copy the name from the 'particular' column."* The file corroborates this: 26 rows, of which **20 are plain copies of the narrative, 3 are blank, and 1 required human knowledge** (`By Trf Reversal of a transaction wrongly posted` → *Milele Agroprocessing Limited*, unrecoverable from the text). The file also carries **a repeated page-break header row inside the data** (`Particulars | NAME`) with a stray `Development Bank` fragment beneath it — a copy-paste artefact no generated column produces.

**Consequence — the architectural conclusion of this whole data exercise:** *neither* interest *nor* fees can be attributed to customers from the general ledger. Interest is aggregated before posting (§3.5.1); fees reach the GL with the customer only ever as unstructured prose. **The loan module is the sole source of customer-grain revenue data for both.** Requesting a `NAME` column on the 2025 fee spools would only be asking MAIIC to hand-type it again — the very manual mapping that would not survive audit (open item #25, rewritten).
...
Source: verified commit history on branch **`eir_revenue_recognition`** (pushed, not yet merged to `master`): `b50f708 → 4027641 → 4c4fc42 → 2dfa5b7 → [Extract A/C intake, 2026-08-12] → e497db7 (2026-08-18) → ac030a6 / e553b48 / ba9f682 (2026-08-19) → 47a5d88 / 676d6e6 / 72d4388 / 1f11004 / 7da1100 (2026-09-01) → af7c6e2 (2026-09-02)`. Status below is from direct code inspection on 2026-09-10 on a clone with dependencies installed; the focused EIR and ECL suites were run and pass, so ✅ entries are confirmed green, not only present. The build log in `docs/Development_of_EIR.md` carries a September addendum with the same facts.
...
| **2.8 — Trial-balance corpus ingestion** *(built 2026-09-01, `47a5d88`)* | `TrialBalanceImportService` + `eir:import-trial-balances` (grand-total checksum, period stamp, AFS pre-closing December sheet) → `gl_trial_balance_lines`; `GlAccountScope` seeded (E-Banker vs QuickBooks charts, EIR scope); `TrialBalanceMovementService` applies the cumulative-YTD rule on read (January whole, balance sheet never differenced, missing prior month refused) | ✅ **Built** — 11 movement tests incl. twelve differenced months summing to the audited annual total |
...
| **7 — Accuracy upgrades** | Actuals import → `cash_source=IMPORTED`; CCF model | 🔲 **Not started** — `EirRevenueService` already prefers imported actuals when the actuals window covers the month, so the remaining work is the end-to-end actuals feed and the CCF model |
...
**Test files as at 2026-09-10:** `EclDiscountRateServiceTest.php`, `TimePhasedEclServiceTest.php`, `EirRevenueServiceTest.php` (13 cases), `EirGlReconciliationServiceTest.php`, `EirCoverageServiceTest.php`, `EirCalculationWorkflowTest.php`, `EirIntakeServicesTest.php`, `EirIntakeStatusTest.php`, `FeeRuleSweepTest.php`, `StagingClassifierTest.php`, `TrialBalanceImportServiceTest.php`, `TrialBalanceMovementServiceTest.php`, `EirReadinessServiceTest.php`, plus `tests/Unit/Eir/{CalculateEirServiceTest,DateSensitiveEirServiceTest,ScheduleGeneratorServiceTest,MappedFileReaderTest}.php`. Run from this clone with dependencies installed: the focused EIR and ECL suites pass. CI (`.github/workflows/ci.yml`) runs `tests/Unit/Eir tests/Unit/Support tests/Feature/Eir` on every push.
...
| 10 | **Fee attribution to customers exists only as free-text `Particulars` narrative** (19–22% exact auto-match). No structured customer field reaches the GL for fees — the 2026 `NAME` column was hand-typed by MAIIC, not system-generated (§3.5.2). Together with limitation #9 this means **the GL cannot attribute either interest or fees to a customer** | Disclose as a source-system constraint; any interim mapping table must be documented and reviewable | Kundai / Barry | Loan-module fee extract delivered (open items #24/#25) |
