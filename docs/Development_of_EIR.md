...
**Column type is inferred and the transform pre-selected.** The transform dropdown previously defaulted to text for every column, so an operator either set 21 of them by hand or silently imported dates as strings. The type now comes from the values — the only thing that knows `as_of_date` and `balance_snapshot_date` are Excel serials rather than numbers — and the field-name defaults fill in where the values are inconclusive. On the delivered Extract A: 6 columns typed date (all correct, including both serial columns), 5 number, 21 text, 3 empty. Identifiers stay text deliberately; magnitude cannot separate an account number from an amount, so only a decimal point or thousands separator promotes a column to number. The bias costs nothing — the import services clean their own values — while an identifier cast to a number loses its leading zeros permanently. Percent is never suggested: whether `32.10` means 32.1% or 3,210% cannot be read from the values, and guessing moves a solved rate by two orders of magnitude.

The typing also surfaced three columns that are **entirely empty** in the delivered file: `writeoff_date`, `writeoff_amount` and `restructure_date` (consistent with `writeoff_flag` = `N` ×362). No facility carries a restructure date, which matters because modification accounting is a phase of its own — either no facility in this population has been restructured, or the extract does not carry it.
