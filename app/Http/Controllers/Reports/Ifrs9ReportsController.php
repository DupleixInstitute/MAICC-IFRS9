...
class Ifrs9ReportsController extends Controller
{
    ...
    /**
     * The user supplies their own shock percentages for PD, LGD and a
     * combined PD+LGD shock; ECL is recomputed for each. No hardcoded
     * scenarios — driven by query params from the on-page controls form.
     */
    public function sensitivity(Request $request)
    {
        ...
        return $this->respond(['key' => 'sensitivity', 'period' => $period,
            ...
            'controls' => [
                ...
                'fields' => [
                    ['name' => 'pd_shocks',       'label' => 'PD shocks (%)',       'value' => implode(',', $pdShocks)],
                    ['name' => 'lgd_shocks',      'label' => 'LGD shocks (%)',      'value' => implode(',', $lgdShocks)],
                    ['name' => 'combined_shocks', 'label' => 'PD & LGD shocks (%)', 'value' => implode(',', $combinedShocks)],
                ],
            ],
            'kpis' => [
